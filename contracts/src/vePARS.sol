...
/**
 * @title vePARS - Vote-Escrowed PARS
 * @notice Lock PARS to receive voting power for governance
 * @dev Based on vLUX pattern from @luxfi/contracts
 *
 * VE-TOKENOMICS:
 * ┌─────────────────────────────────────────────────────────────────────────────┐
 ...
 * │                                                                             │
 * │  vePARS = PARS_amount * (lock_time / MAX_LOCK_TIME)                         │
 * │                                                                             │
 * │  Example:                                                                   │
 * │  - 1000 PARS locked 4 years = 1000 vePARS                                   │
 * │  - 1000 PARS locked 1 year  = 250 vePARS                                    │
 * │  - 1000 PARS locked 1 week  = ~5 vePARS                                     │
 * │                                                                             │
 ...
 * │  - Participate in gauge weight voting                                       │
 * └─────────────────────────────────────────────────────────────────────────────┘
 */
contract vePARS is ReentrancyGuard {
    ...
    struct LockedBalance {
        uint256 amount;     // Locked PARS amount
    ...
    function withdraw() external nonReentrant {
        ...
        pars.safeTransfer(msg.sender, amount);
    ...
    /// @return Voting power (vePARS balance)
    ...
    function _depositFor(
        ...
        if (amount > 0) {
            pars.safeTransferFrom(msg.sender, address(this), amount);
        }
